...
CFG = {
    ...
    'num_classes': 1000,
    'model_name': 'resnet50',
    'info': 'ResNet_AdamW_setting2_0.001',
    'ifval': False,
    'model_path': 'ResNet_AdamW_setting2_0.001.pth'
}
...
class ImageNetDataset(Dataset):
    def __init__(self, root, part='train', transforms=None):
        ...
        for i in range(len(mycsv['image_id'])):
            self.images.append('/data3'+mycsv['image_id'][i][3:])
...
def test_one_epoch(model, val_loader, device):
    model.eval()
    image_preds_all = []
    image_targets_all = []
...
if __name__ == '__main__':
    ...
    model = timm.create_model(model_name=CFG['model_name'], num_classes=CFG['num_classes'], pretrained=False)
    ...
    if CFG['ifval'] == True:
        ...
        answer = test_one_epoch(model, val_loader, device)
        print(answer)
        exit()
